...
def create_llm_client(preference: str = "auto"):
...
def build_agent(llm_preference: str) -> CodeReviewAgent:
    runtime = ToolRuntime()
    runtime.register("echo_tool", echo_tool)

    client = create_llm_client(llm_preference)
...
def run_agent(prompt: str, llm_preference: str, stream_callback=None) -> str:
    diff_ctx = collect_diff_context()
    agent = build_agent(llm_preference)
    augmented_prompt = f"{prompt}\n\n[Diff Summary]\n{diff_ctx.summary}"
    return asyncio.run(
        agent.run(augmented_prompt, files=diff_ctx.files, stream_observer=stream_callback)
    )
...
def main() -> None:
    ...
    tk.Label(root, text="Prompt").pack(anchor="w", padx=8, pady=(8, 0))
    prompt_box = scrolledtext.ScrolledText(root, height=8)
    prompt_box.insert(
        tk.END,
        "请审查下面的代码。在需要时调用 echo_tool (提示: tool:).\n",
    )
    prompt_box.pack(fill=tk.BOTH, expand=False, padx=8, pady=4)

    model_label = tk.Label(root, text="LLM Provider")
    model_label.pack(anchor="w", padx=8, pady=(4, 0))
    ...
    options.append("mock")
    tk.OptionMenu(root, model_var, *options).pack(anchor="w", padx=8, pady=(0, 8))

    tk.Label(root, text="Result").pack(anchor="w", padx=8, pady=(8, 0))
    result_box = scrolledtext.ScrolledText(root, height=14)
    result_box.pack(fill=tk.BOTH, expand=True, padx=8, pady=(0, 8))

    event_queue: "queue.Queue[Dict[str, Any]]" = queue.Queue()

    def worker(prompt_text: str, preference: str) -> None:
        def observer(event: Dict[str, Any]) -> None:
            event_queue.put({"type": "delta", "payload": event})

        try:
            result = run_agent(prompt_text, preference, observer)
    ...
    def on_run() -> None:
        ...
        thread = threading.Thread(
            target=worker, args=(prompt, model_var.get()), daemon=True
        )
        thread.start()
        poll_queue()
