...
SYSTEM_PROMPT_PLANNER = (
    ...
    "- rule_confidence: 规则置信度(0-1，>=0.8高风险，0.5-0.8中风险，<0.5低风险，0.0表示无法确定，需要进行把握)"
    "- tags: 变更标签（安全/配置/噪音等）"
    "- metrics: 行数/hunk_count 等规模信息"
    "- line_numbers: new_compact/old_compact 行号摘要"
    "- rule_extra_requests: 规则建议的额外上下文（previous_version/callers/search，可为空）"
    ...
    "规则：高风险标签（security_sensitive/config_file/routing_file）不得 skip；按重要性筛选，避免全选；不生成审查结论，不调用工具，不编造 unit；如有 rule_extra_requests 可直接使用。"
)
