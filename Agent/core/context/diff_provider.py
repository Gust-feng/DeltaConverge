...
def build_markdown_and_json_context(
    ...
    lines.append("- 字段速览：")
    lines.append("  - `rule_context_level`: 规则建议的上下文粒度（diff_only/function/file_context）")
    lines.append("  - `rule_confidence`: 规则置信度(0-1)，>=0.8高置信度，0.5-0.8中等，<0.5低置信度")
    lines.append("  - `rule_notes`: 规则备注，说明匹配的规则类型")
    lines.append("  - `tags`: 变更标签（安全/配置/噪音等）")
    lines.append("  - `agent_decision`: 规则层决策摘要")
    lines.append("")
