...
### 输入
- Git diff输出（支持working/staged/pr/auto四种模式）

### 处理流程

```python
# Agent/core/context/diff_provider.py
def collect_diff_context(mode: DiffMode = DiffMode.AUTO, cwd: Optional[str] = None) -> DiffContext:
...
1. **获取原始diff**：根据模式调用对应的git命令
   ...
   - `pr`: `git fetch origin {base_branch}` 后执行 `git diff origin/{base_branch}...HEAD`
   - `auto`: staged → working → pr（按仓库状态自动选择）
