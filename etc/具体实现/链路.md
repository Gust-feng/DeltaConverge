...
- `rule_extra_requests`：规则侧建议的附加上下文（如 previous_version/callers/search），可为空。
- `line_numbers`：行号摘要（new_compact/old_compact）。
- `agent_decision`（精简版）：仅保留 `context_level`、`priority`，用于展示/排序。

已去除冗余：不再下传 rule_suggestion/full agent_decision/diff 正文。
    ...
    "plan": [
      {
        ...
        "extra_requests": [{"type": "callers|previous_version|search", "details": "..."}],
        "skip_review": false,
        "reason": "..."
      }
    ]
...
- 遍历 fused plan 中未 skip 的 unit：
  ...
  - 处理 extra_requests：
    ...
    - `callers`/`search`：rg/git grep 搜索关键符号/文本，返回片段。
- 输出 bundle item 示例：
```json
{
  "unit_id": "...",
...
- review_index 也精简：`agent_decision` 仅 context_level/priority，去掉 rule_suggestion/rule_notes/diff 正文。
