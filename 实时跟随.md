...
- `log/`：运行时会话日志（如 `agent_session_*.log`、`*_stream_chat_*.log`），用于追踪 tokens 用量与工具调用。
...
- GUI tokens 展示：按 call_index 统计单次用量并汇总 session_total，去除流式重复上报导致的虚高；提示格式与 CLI 统一为编号样式。
- 会话日志：agent_session 日志清晰记录每次请求/响应与工具结果，便于追踪 tokens 增长与上下文传递。

## 2025-11-21

- 抽象 UI 服务层：新增 `Agent/ui/service.py`，提供 `create_llm_client` / `run_review` / `UsageAggregator`，UI 可根据该内核自由实现（Tk/Web 等）。
- 精简 GUI：删除本地 Agent/LLM 组装与冗余统计，直接调用服务层；tokens 显示依赖 UsageAggregator；事件类型（delta/usage_summary）保持透传。
- Agent：提取 `_extract_usage`，统一从 normalized/raw chunks 回溯 usage，减少内联重复。
- LLM 客户端：流式解析时兼容 `choices[0].usage`，防止 usage 丢失；确保 GUI 能显示 tokenss。
...
- 问题留存：流式审查仍未记录 usage（需在 StreamProcessor 补取 stop chunk 的 usage 字段）；大规模 review_index 导致规划请求 prompt_tokenss ~41K，必要时应截断或分批。

> 使用修改后的审查系统进行自我审查，居然打掉六十万tokens🫨

## 2025-11-26

- 融合策略收紧：`fuse_plan` 在 planner 为空/异常时只保留规则高置信或安全/配置高风险单元，其余统一标记 skip，防止退回“全量上下文”；补充原因字段方便排查。
- 上下文可观测性：`Agent/ui/service.py` 记录 context bundle 的体积、截断次数和层级分布到日志，便于发现上下文膨胀。
- 规则占位清理：`call_context_agent` 改为直接返回规则兜底决策，不再抛出 NotImplemented。
- 感知层增强行号与体积控制：`diff_collector.py` 为每个 hunk 采集真实行号（compact 行号 + 带行号 diff），合并 hunk 时保留行号；LLM 输出只保留 `new_compact/old_compact`，避免巨型数字列表；CLI `main` 移到文件末尾修复 NameError；语言识别扩展到 TS/TSX/Ruby 等。
- 上下文调度瘦身：`context_scheduler.py` 在 diff 前追加 `@@ file:行号 @@` 位置头，超长 diff 按行截断，空上下文/重复 callers 去除，调用方片段去重，保留带行号 diff；行号仅保留于 meta，减少冗余。
- 工具输出行号：`read_file_hunk` 增加 `snippet_with_line_numbers`，方便模型引用精确行号。
- 类型/健壮性修正：GLM 适配器流式路径安全写入 `raw`；diff_collector 规避 TypedDict 类型警告、AST 行号访问安全化；规划融合/上下文链路保持不变。
- 日志上下文工具：新增 `Agent/core/logging/utils.py` 提供 UTC ISO 时间戳与递归截断 `safe_payload`，统一处理大字段与脱敏。
- 流水线日志规范化：`PipelineLogger` 统一 JSONL 格式（event/stage/status/ts/trace_id/uptime_ms），文件名含 trace_id，对 diff/context/messages 等大字段默认 redacted。
- 会话级 API 日志：`APILogger` 改为同一 trace_id 复用一份 JSONL，按顺序记录多轮 LLM/工具请求与响应；人类摘要文件同样按 trace_id 复用，避免一轮审查生成多个文件。
- 流式日志抑制：默认不逐 chunk 落盘，`RESPONSE_SUMMARY` 补充 chunk_count/chunks_logged；需要调试时可开启采样开关。
- LLM 客户端：Moonshot/GLM/Bailian 流式 summary 追加 chunk_count，便于回放；日志 label 字段记录来源（如 stream_chat/agent_session）。
- 回退监控：新增 `Agent/core/logging/fallback_tracker.py`，提供线程安全的回退计数与示警，后续可在 diff/IO 降级路径调用。

- 工具白名单与默认集：`Agent/tool/registry.py` 将默认工具限定为无害内置（read/search/deps 等），`echo_tool` 需显式选择；新增 `builtin_tool_names` 兼容别名。
- 自动审批策略：服务层在 `auto_approve=False` 时仍自动批准内置安全工具；CLI 默认仅自动批准内置工具，需启用全量自动或交互审批才会运行未来高风险工具（`Agent/examples/run_agent.py`、`Agent/ui/service.py`）。
- UI/接口默认值：GUI “自动执行工具” 默认关闭，Web API `autoApprove` 默认 false，Web 前端复选框默认未选（`Agent/examples/gui.py`、`Agent/web/server.py`、`Agent/web/static/index.html`）。
- 文档同步：接口说明更新默认审批策略，强调内置安全工具自动执行、未来风险工具需显式开启或走审批（`Agent/examples/接口说明.md`）。
- GUI 布局：侧栏工具列表改为填充剩余高度，消除下方留白，整体更紧凑（`Agent/examples/gui.py`）。
